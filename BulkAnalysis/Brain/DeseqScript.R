---
title: "Differential Gene Expression Analysis: Young BEC vs Aged BEC"
...
setwd("~/Desktop/Brain")
# read in count data
CountData = read.csv('CombinedBrainCounts.csv',header=TRUE, row.names=1)
head(CountData)
library(dplyr)
# Reshuffle the columns 
CountData1 = CountData %>% select(YB1 ,YB3 ,YB4,OB1,OB3,OB4)
# Rename the sample names
colnames(CountData1) = c('YB1','YB2','YB3','OB1','OB2','OB3')
...
BrainMetadata = data.frame(row.names,condition,libType, row.names=1)
...
all(rownames(BrainMetadata) %in% colnames(CountData1))
# This should be True
all(rownames(BrainMetadata)==colnames(CountData1))
# This should be true too
# Now make sure the condition have two levels (Young and Old)
BrainMetadata$condition
# 2 levels Old and Young
# Now construct a DeSeq2 Data set
dds = DESeqDataSetFromMatrix(countData=CountData1, colData=BrainMetadata, design = ~ condition)
...
# It says 415 genes at an alpha level of 0.1
...
# 273 genes were significant after multiple test correction
...
colnames(NormalizedCountData) = c("YB1_norm","YB2_norm","YB3_norm","OB1_norm","OB2_norm","OB3_norm","Gene_id")
...
colnames(Log2_transformed) = c("YB1_lognorm","YB2_lognorm","YB3_lognorm","OB1_lognorm","OB2_lognorm","OB3_lognorm")
...
New3 = New2 %>% select_('Gene_id','hgnc_symbol',"YB1","YB2","YB3","OB1","OB2","OB3","YB1_norm","YB2_norm","YB3_norm","OB1_norm","OB2_norm","OB3_norm","YB1_lognorm","YB2_lognorm","YB3_lognorm","OB1_lognorm","OB2_lognorm","OB3_lognorm", "baseMean","log2FoldChange",'pvalue',"padj")
# Rename the hgnc_symbol as just the Gene_symbol
colnames(New3)[2] = 'Gene_symbol'
# Now save this result
write.csv( New3, file="Brain_Counts_p_values.csv" )
